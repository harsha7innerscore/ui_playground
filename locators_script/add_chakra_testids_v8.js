 ...
 *
 * New features in v8:
 * 1. Enhanced support for className and sx prop values for test ID generation
 * 2. Default output path now points to locators_script/results
 * 3. Improved parsing of CSS-in-JS and styled-components patterns
 * 4. Better extraction of semantic meaning from class names
 * 5. Smart detection of CSS frameworks like Tailwind, Bootstrap, etc.
 * 6. All v7 features including semantic naming and component relationship detection
 *
 * Usage: node add_chakra_testids_v8.js <input-file.jsx> [options]
 * Options:
...
if (!inputFile) {
  console.error('Please provide an input file: node add_chakra_testids_v8.js <input-file.jsx> [options]');
  process.exit(1);
}
...
function processFileWithPaths(inputFilePath, outputFilePath) {
  ...
  gatherComponentContext(ast, commentMap, hierarchyMap, classNameMap, sxPropMap, loopMap, keyPropMap, keyExpressionMap, idPropMap, wrapperChildMap, roleMap, semanticGroupMap, interactiveElementMap, componentContextMap, frameworks, options);

  // Process the AST and add data-testid attributes
  const { addedCount, componentTypesCount } = addTestIds(ast, chakraComponents, customComponents, htmlElements, options, counters, testIdCache, commentMap, hierarchyMap, classNameMap, sxPropMap, loopMap, keyPropMap, keyExpressionMap, idPropMap, wrapperChildMap, roleMap, semanticGroupMap, interactiveElementMap, componentContextMap);
...
function initializeDataStructures() {
  return {
    ...
    sxPropMap: new Map(), // NEW in v8: Map to store sx prop values
 ...
 */
function gatherComponentContext(ast, commentMap, hierarchyMap, classNameMap, sxPropMap, loopMap, keyPropMap, keyExpressionMap, idPropMap, wrapperChildMap, roleMap, semanticGroupMap, interactiveElementMap, componentContextMap, frameworks, options) {
  traverse(ast, {
    enter(path) {
  ...
  traverse(ast, {
    JSXElement(path) {
      buildComponentContext(path, hierarchyMap, classNameMap, sxPropMap, commentMap, roleMap, semanticGroupMap, interactiveElementMap, componentContextMap, frameworks, options);
    }
  });
}
...
/**
 * NEW in v8: Extract sx prop from element
 */
 ...
 */
function buildComponentContext(path, hierarchyMap, classNameMap, sxPropMap, commentMap, roleMap, semanticGroupMap, interactiveElementMap, componentContextMap, frameworks, options) {
  const node = path.node;
 ...
 * NEW in v8: Enhanced to handle different CSS frameworks
 */
...
/**
 * NEW in v8: Extract semantic info from sx prop
 */
function getSemanticMeaningFromSx(sxProps) {
  if (!sxProps || typeof sxProps !== 'object') return '';

  // Check for common props that indicate purpose/role
  const purposeProps = ['variant', 'color', 'size', 'component', 'display', 'position'];
  for (const prop of purposeProps) {
    if (sxProps[prop]) {
      return sxProps[prop];
    }
  }
  ...
  // Combine other meaningful props
  const meaningfulProps = [];
  const importantProps = ['backgroundColor', 'background', 'color', 'width', 'height'];

  for (const prop of importantProps) {
    if (sxProps[prop]) {
      meaningfulProps.push(sxProps[prop]);
    }
  }

  return meaningfulProps.length > 0 ? meaningfulProps.join('-') : '';
}
...
function generateTestId(componentName, attributes, parentPath, path, options, counters, testIdCache,
  commentMap, hierarchyMap, classNameMap, sxPropMap, loopMap, keyPropMap, keyExpressionMap, idPropMap,
  ...
  if (context) {
    ...
    // NEW in v8: Use className with framework-specific meaning
    else if (context.className && options.classBased) {
    ...
    // NEW in v8: Use sx prop for meaningful naming
    else if (Object.keys(context.sx).length > 0 && options.sxBased) {
  ...
  if (!attributeFound) {
    ...
    // 3. NEW in v8: Check for sx prop
    if (!attributeFound && options.sxBased) {
    ...
    if (!attributeFound && options.classBased) {
      ...
      if (classNameAttr) {
        ...
        if (classNames.length > 0) {
          // NEW in v8: Enhanced semantic extraction
    ...
    // 5. Check for semantic props for UI components
    if (!attributeFound) {
    ...
    // 6. For buttons, links and clickable elements, look for onClick handlers
    if (!attributeFound &&
    ...
    // 7. For Image components, use src context
    ...
    // 8. Use text content if available and option is enabled
    ...
    // 9. Check for comments if option is enabled
