 ...
 *
 * New features in v6:
 * 1. Improved handling of dynamic keys with template literals
 * 2. Properly preserves variables in template literals for data-testid
 * 3. Uses the exact same key value pattern for data-testid when available
 * 4. Better extraction and handling of dynamic parts in keys
 * 5. All v5 features including improved loop context, better naming, and user-specified prefix
 *
 * Usage: node add_chakra_testids_v6.js <input-file.jsx> [options]
 * Options:
...
if (!inputFile) {
  console.error('Please provide an input file: node add_chakra_testids_v6.js <input-file.jsx> [options]');
  process.exit(1);
}
...
function processFile() {
  ...
  // Map to store loop info for components
  const loopMap = new Map();
  // Map to store key props for components
  const keyPropMap = new Map();
  // Map to store raw key expression for components (NEW in v6)
  const keyExpressionMap = new Map();
  // Map to store id props for components
  const idPropMap = new Map();
  // Map to store wrapper-child relationships
    ...
    enter(path) {
      ...
      if (path.isJSXElement()) {
        ...
        if (openingElement && openingElement.attributes) {
          ...
          if (keyAttr) {
            let keyValue = '';
            // NEW in v6: Store the raw expression for potential direct use
            if (t.isJSXExpressionContainer(keyAttr.value)) {
              keyExpressionMap.set(path.node, keyAttr.value.expression);
            }
            ...
            if (t.isStringLiteral(keyAttr.value)) {
              ...
              if (t.isIdentifier(expr)) {
                ...
                try {
                  // NEW in v6: Enhanced template literal extraction
  ...
  // NEW in v6: Function to determine if we should use dynamic test ID
  function shouldUseDynamicTestId(node) {
  ...
  // NEW in v6: Create JSX expression from key expression for data-testid
  function createDynamicTestId(node, prefix = '') {
    const keyExpr = keyExpressionMap.get(node);
    if (!keyExpr) return null;

    // If prefix is needed, create a template literal with the prefix
    if (prefix) {
      // Create a template literal that adds the prefix to the key value
      const prefixLiteral = t.stringLiteral(prefix);
  ...
  function generateTestId(componentName, attributes, parentPath, path) {
    ...
    // NEW in v6: Check if we should use dynamic test ID based on key
    if (isLooped && parentPath && shouldUseDynamicTestId(parentPath.node)) {
    ...
    // 2. Second priority: Check for className
    if (!attributeFound) {
    ...
    // 9. Use wrapper-childcomponentName format for looped items
  ...
  traverse(ast, {
    JSXOpeningElement(path) {
      ...
      const testIdResult = generateTestId(componentName, node.attributes, parentPath, path);

      // NEW in v6: Handle dynamic test IDs
      if (typeof testIdResult === 'object' && testIdResult.isDynamic) {
        // Create a dynamic data-testid using the key expression
        const dynamicExpr = createDynamicTestId(testIdResult.node, options.prefix);
        if (dynamicExpr) {
