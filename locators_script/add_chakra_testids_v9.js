 ...
 *
 * New features in v9:
 * 1. Improved component fingerprinting to prevent duplicate test IDs
 * 2. Enhanced extraction and prioritization of sx prop values
 * 3. Better priority system for text content in descriptive IDs
 * 4. Path-based differentiation for similar components
 * 5. Improved context preservation across related components
 * 6. Advanced uniqueness guarantees for all generated test IDs
 * 7. All v8 features including className and sx support
 *
 * Usage: node add_chakra_testids_v9.js <input-file.jsx> [options]
 * Options:
...
if (!inputFile) {
  console.error('Please provide an input file: node add_chakra_testids_v9.js <input-file.jsx> [options]');
  process.exit(1);
}
...
function initializeDataStructures() {
  return {
    ...
    pathMap: new Map(), // NEW in v9: Map to store component path information
    childTextMap: new Map() // NEW in v9: Map to store text from child elements
  };
}
...
function gatherComponentContext(
  ...
  traverse(ast, {
    enter(path) {
      ...
      if (path.isJSXElement()) {
        extractElementAttributes(path, classNameMap, sxPropMap, keyPropMap, keyExpressionMap, idPropMap, roleMap, frameworks, options);
        ...
        // NEW in v9: Extract text from child elements for better context
 ...
 */
function extractElementAttributes(path, classNameMap, sxPropMap, keyPropMap, keyExpressionMap, idPropMap, roleMap, frameworks, options) {
  const openingElement = path.node.openingElement;
  if (openingElement && openingElement.attributes) {
    // Extract className
    if (options.classBased) {
      extractClassName(path.node, openingElement.attributes, classNameMap, frameworks);
    }

    // Extract sx prop
    if (options.sxBased) {
      extractSxProp(path.node, openingElement.attributes, sxPropMap, frameworks);
    }
...
/**
 * Extract sx prop from element
 */
function extractSxProp(node, attributes, sxPropMap, frameworks) {
  const sxAttr = attributes.find(
    attr => t.isJSXAttribute(attr) && (attr.name.name === 'sx' || attr.name.name === 'style')
  );

  if (sxAttr) {
    // Handle different types of sx values
    if (t.isJSXExpressionContainer(sxAttr.value) && sxAttr.value.expression) {
      if (t.isObjectExpression(sxAttr.value.expression)) {
        // Handle object literal: sx={{ color: 'primary.main', display: 'flex' }}
        try {
          const properties = sxAttr.value.expression.properties;
          const sxValues = {};
            ...
            if (t.isObjectProperty(prop)) {
              if (t.isIdentifier(prop.key)) {
                ...
                if (t.isStringLiteral(prop.value)) {
                  ...
                  if (prop.value.property && t.isIdentifier(prop.value.property)) {
                    value = prop.value.property.name;
                  }
                } else if (t.isObjectExpression(prop.value)) {
                  // Handle nested objects like _hover: { bg: 'green.500' }
                  try {
                    const nestedProps = [];
                    prop.value.properties.forEach(nestedProp => {
                      if (t.isObjectProperty(nestedProp) && t.isIdentifier(nestedProp.key)) {
                        let nestedValue = '';
                        if (t.isStringLiteral(nestedProp.value)) {
                          nestedValue = nestedProp.value.value;
                          nestedProps.push(`${nestedProp.key.name}:${nestedValue}`);
                        }
                      }
                    });

                    if (nestedProps.length > 0) {
                      value = `{${nestedProps.join(',')}}`;
                    }
                  } catch (e) {
                    // Unable to extract nested properties
                  }
                }
          ...
          if (Object.keys(sxValues).length > 0) {
            sxPropMap.set(node, sxValues);
          }
        } catch (e) {
          // Unable to extract
        }
      } else if (t.isIdentifier(sxAttr.value.expression)) {
        // Handle variable reference: sx={styles}
        const varName = sxAttr.value.expression.name;
        sxPropMap.set(node, { _reference: varName });
      }
    }
  }
}
...
/**
 * Check if a className is a Tailwind CSS class
 */
function isTailwindClass(className) {
  ...
  const tailwindPatterns = [
    ...
    /^(mt|mb|ml|mr|mx|my|pt|pb|pl|pr|px|py)-[0-9]+$/,
  ...
  return classes.some(cls => tailwindPatterns.some(pattern => pattern.test(cls)));
}

/**
 * Check if a className is a Bootstrap class
 */
function isBootstrapClass(className) {
  ...
  const bootstrapPatterns = [
    /^(container|row|col|col-\w+)/,
    /^(btn|form-control|card|nav|navbar)/,
    /^(d-flex|d-grid|d-block|d-inline|d-none)/,
    /^(m|p|mt|mb|ml|mr|pt|pb|pl|pr)-[0-9]+$/,
    /^(text|bg|border|rounded|shadow)-/
  ];

  const classes = className.split(/\s+/);
  return classes.some(cls => bootstrapPatterns.some(pattern => pattern.test(cls)));
}
...
function buildComponentContext(
  ...
  try {
    ...
    const nodePath = pathMap.get(node) || [];
    ...
    const context = {
      ...
      childText: childTextMap.get(node) || '',
      parentName: '',
      nestingLevel: 0,
      siblingPosition: 0,
      cssFramework: null,
    ...
    // Detect CSS framework from className
    if (context.className) {
      if (context.className.startsWith('tailwind:')) {
        context.cssFramework = 'tailwind';
        context.className = context.className.substring(9); // Remove "tailwind:" prefix
      } else if (context.className.startsWith('bootstrap:')) {
        context.cssFramework = 'bootstrap';
        context.className = context.className.substring(10); // Remove "bootstrap:" prefix
      }
    }
 ...
 */
function getTextFromJSXElement(node, includeChildren = false) {
  let text = '';
  ...
  if (node.children) {
    node.children.forEach(child => {
      if (t.isJSXText(child)) {
        ...
        if (includeChildren) {
          const childElementText = getTextFromJSXElement(child, includeChildren);
          if (childElementText) {
            text += childElementText + ' ';
          }
        } else {
          // Only process inline text elements when not including all children
...
/**
 * Extract semantic meaning from className
 */
function getSemanticMeaningFromClassName(className, framework) {
  if (!className) return '';
  ...
  if (framework === 'tailwind') {
    return extractTailwindMeaning(classNames);
  } else if (framework === 'bootstrap') {
    return extractBootstrapMeaning(classNames);
  }

  // Default case - look for semantic classes
  return getMeaningfulClassName(classNames);
}

/**
 * Extract semantic meaning from Tailwind classes
 */
function extractTailwindMeaning(classNames) {
  // Priority patterns that indicate component purpose
  const purposePatterns = {
    ...
    header: /header/i,
    footer: /footer/i,
    nav: /nav|navbar|navigation/i,
    sidebar: /sidebar|aside/i,
    main: /main/i,
    section: /section/i
  };

  // Check if any class indicates component purpose
  for (const [purpose, pattern] of Object.entries(purposePatterns)) {
    if (classNames.some(cls => pattern.test(cls))) {
      return purpose;
    }
  }

  // Extract semantic info from Tailwind utility classes
  const tailwindInfo = [];

  // Check for color
  const colorClass = classNames.find(cls => /^(?:bg|text|border)-(?!opacity|transparent|current|black|white)([\w-]+)/.test(cls));
  if (colorClass) {
    const colorMatch = colorClass.match(/^(?:bg|text|border)-([\w-]+)/);
    if (colorMatch && colorMatch[1]) tailwindInfo.push(colorMatch[1]);
  }

  // Check for size
  const sizeClass = classNames.find(cls => /^(w|h|text)-(\w+)/.test(cls));
  if (sizeClass) {
    const sizeMatch = sizeClass.match(/^(w|h|text)-(\w+)/);
    if (sizeMatch && sizeMatch[2]) tailwindInfo.push(sizeMatch[2]);
  }

  // Check for display type
  const displayClass = classNames.find(cls => /^(flex|grid|block|inline|table)/.test(cls));
  if (displayClass) {
    tailwindInfo.push(displayClass);
  }

  if (tailwindInfo.length > 0) {
    return tailwindInfo.join('-');
  }

  // Filter out utility classes
  const nonUtilityClass = classNames.find(cls =>
    !(/^(m|p|w|h|text|bg|border|flex|grid|block|inline|table|rounded|shadow|opacity|z|transition|transform|scale|rotate|translate|skew|origin|overflow|cursor|select|pointer|user|visible|invisible|truncate|sr-only)/.test(cls))
  );

  return nonUtilityClass || '';
}

/**
 * Extract semantic meaning from Bootstrap classes
 */
function extractBootstrapMeaning(classNames) {
  // Check for component classes first
  const componentPatterns = {
    button: /btn($|-\w+)/,
    card: /card($|-\w+)/,
    alert: /alert($|-\w+)/,
  ...
  for (const [component, pattern] of Object.entries(componentPatterns)) {
    const match = classNames.find(cls => pattern.test(cls));
  ...
  for (const [layout, pattern] of Object.entries(layoutPatterns)) {
    const match = classNames.find(cls => pattern.test(cls));
    if (match) return layout;
  }

  // Look for non-utility classes
  return classNames.find(cls =>
    !(/^(d-|m-|p-|text-|bg-|border-|w-|h-|position-|display-|flex-|float-|font-|rounded|shadow)/.test(cls))
  ) || '';
}
...
/**
 * Extract semantic info from sx prop
 */
function getSemanticMeaningFromSx(sxProps) {
  if (!sxProps || typeof sxProps !== 'object') return '';

  // Priority order of meaningful prop keys
  const priorityProps = [
    'variant',         // Top priority - directly indicates component variant
    'colorScheme',     // Color scheme often indicates purpose
    'color',           // Color can indicate purpose or state
    'backgroundColor', // Background color can be meaningful
    'bg',              // Short form of backgroundColor
    'status',          // Status is often meaningful
    'size',            // Size can differentiate between types
    'type',            // Type is often meaningful for inputs etc.
    'role',            // Role indicates purpose
    'width',           // Width can be meaningful for layout components
    'height',          // Height can be meaningful for layout components
    'position',        // Position can indicate purpose
    'display',         // Display mode can indicate purpose
    'justifyContent',  // Layout intent can provide context
    'alignItems',      // Alignment can provide context
    'fontWeight',      // Font weight can indicate purpose (heading, emphasis)
    'fontSize'         // Font size can indicate purpose
  ];

  // Try to find the highest priority prop value
  for (const prop of priorityProps) {
    if (sxProps[prop] && typeof sxProps[prop] === 'string') {
      // For color values, clean up format like "green.500" to just "green"
      if (['color', 'backgroundColor', 'bg', 'colorScheme'].includes(prop)) {
        const colorValue = sxProps[prop];
  ...
  // If there's a reference to a styles object
  if (sxProps._reference) {
    return sxProps._reference;
  }

  // Combine multiple props if no single high-priority prop was found
  const meaningfulValues = [];

  // Extract meaningful values in priority order
  for (const prop of priorityProps) {
    if (sxProps[prop] && typeof sxProps[prop] === 'string') {
      // For color values, clean up format like "green.500" to just "green"
      if (['color', 'backgroundColor', 'bg', 'colorScheme'].includes(prop)) {
        const colorValue = sxProps[prop];
        if (colorValue.includes('.')) {
          meaningfulValues.push(colorValue.split('.')[0]);
        } else {
          meaningfulValues.push(colorValue);
        }
      } else {
        meaningfulValues.push(sxProps[prop]);
      }

      // Take at most 2 values
      if (meaningfulValues.length >= 2) break;
    }
  }
...
/**
 * Create a path-based fingerprint for the component
 */
function createPathFingerprint(component, pathMap, nestingLevel, siblingPosition) {
  const path = pathMap.get(component) || [];
  return `${path.join('/')}:${nestingLevel}:${siblingPosition}`;
}
...
function generateTestId(
  ...
  // Check for cached test ID for similar components (with path info for v9)
  let componentFingerprint;
  if (options.pathContext && parentPath && pathMap.get(parentPath.node)) {
    // Use path-based fingerprint for better uniqueness
    const context = componentContextMap.get(parentPath.node) || {};
    componentFingerprint = createPathFingerprint(
      parentPath.node,
      pathMap,
      context.nestingLevel || 0,
      context.siblingPosition || 0
    );
  } else {
    // Fallback to attribute-based fingerprint
    componentFingerprint = createComponentFingerprint(componentName, attributes, parentPath);
  }
  ...
  if (testIdCache.has(componentFingerprint)) {
    ...
    if (isInLoop(parentPath ? parentPath.node : null, path, loopMap, keyPropMap)) {
  ...
  if (context) {
    // NEW in v9: Higher priority for text content if prioritizeText option is enabled
    if (options.prioritizeText && context.text) {
      suffix = sanitizeForTestId(context.text);
      attributeFound = true;
      if (options.verbose) console.log(`Using text content with high priority: ${suffix}`);
    }
    // NEW in v9: Also consider child text for better context
    else if (options.prioritizeText && context.childText) {
      suffix = sanitizeForTestId(context.childText);
      attributeFound = true;
      if (options.verbose) console.log(`Using child text content: ${suffix}`);
    }
    ...
    // NEW in v9: Enhanced sx priority - check sx props before className
    else if (options.sxBased && Object.keys(context.sx).length > 0) {
      const sxMeaning = getSemanticMeaningFromSx(context.sx);
      if (sxMeaning) {
        suffix = sxMeaning;
        attributeFound = true;
        if (options.verbose) console.log(`Using sx prop semantic meaning: ${suffix}`);
      }
    }
    // Check className after sx props
    else if (options.classBased && context.className) {
      const semanticMeaning = getSemanticMeaningFromClassName(context.className, context.cssFramework);
      if (semanticMeaning) {
        suffix = semanticMeaning;
        attributeFound = true;
        if (options.verbose) console.log(`Using className semantic meaning: ${suffix}`);
      }
    }
    ...
    if (!attributeFound && context.parentName && !suffix.includes(context.parentName.toLowerCase())) {
      suffix = `${context.parentName.toLowerCase()}`;
      attributeFound = true;
      if (options.verbose) console.log(`Using parent component name: ${suffix}`);
    }
  }
  ...
  if (!attributeFound) {
    ...
    // NEW in v9: Text content gets higher priority than sx and className
    if (!attributeFound && options.textBased && options.prioritizeText && parentPath) {
      ...
      if (textContent) {
        ...
        attributeFound = true;
        if (options.verbose) console.log(`Using text content (high priority): ${suffix}`);
      }
    }

    // f) NEW in v9: Check for sx prop with enhanced prioritization
    if (!attributeFound && options.sxBased && parentPath) {
      const sxPropValue = parentPath ? sxPropMap.get(parentPath.node) : null;

      if (sxPropValue && Object.keys(sxPropValue).length > 0) {
        const sxMeaning = getSemanticMeaningFromSx(sxPropValue);
        if (sxMeaning) {
          suffix = sanitizeForTestId(sxMeaning);
          attributeFound = true;
          if (options.verbose) console.log(`Using sx prop: ${suffix}`);
        }
      }
    }

    // g) Check for className
    if (!attributeFound && options.classBased) {
      const classNameAttr = attributes.find(
        attr => t.isJSXAttribute(attr) && attr.name.name === 'className'
      );
      ...
      if (classNameAttr) {
          ...
          try {
            ...
            // Handle any errors in property access
                  ...
                  (classNameAttr.value.expression.callee.name === 'clsx' ||
                   classNameAttr.value.expression.callee.name === 'classNames')) {
          try {
            // Handle clsx or classNames
            const args = classNameAttr.value.expression.arguments;
            const extractedClasses = [];

            args.forEach(arg => {
              if (t.isStringLiteral(arg)) {
                extractedClasses.push(arg.value);
              }
            });

            if (extractedClasses.length > 0) {
              classNames = extractedClasses;
              attributeFound = true;
              if (options.verbose) console.log(`Using classNames from clsx/classNames: ${classNames.join(', ')}`);
            }
          } catch (e) {
        ...
        if (classNames.length > 0) {
          ...
          const cssFramework = classNames.some(cls => isTailwindClass(cls)) ? 'tailwind' :
                              classNames.some(cls => isBootstrapClass(cls)) ? 'bootstrap' : null;

          const semanticMeaning = getSemanticMeaningFromClassName(classNames.join(' '), cssFramework);
          if (semanticMeaning) {
            suffix = semanticMeaning.replace(/[^\w-]/g, '-');
          } else {
    ...
    // h) Check for semantic props for UI components
    if (!attributeFound) {
    ...
    // i) For buttons, links and clickable elements, look for onClick handlers
    if (!attributeFound &&
    ...
    // j) For Image components, use src context
    ...
    // k) Use text content if available and option is enabled
    ...
    // l) Check for comments if option is enabled
  ...
  // NEW in v9: If we still don't have a suffix and pathContext is enabled, use path context
  ...
  // NEW in v9: Keep generating IDs until we find a unique one
  let uniqueId;
    ...
    JSXOpeningElement(path) {
      ...
      if (typeof testIdResult === 'object' && testIdResult.isDynamic) {
        ...
        if (dynamicExpr) {
          ...
          const finalId = options.prefix ? options.prefix + fallbackId : fallbackId;
          node.attributes.push(
            t.jsxAttribute(
              t.jsxIdentifier('data-testid'),
              t.stringLiteral(finalId)
            )
          );
          testIdRegistry.add(finalId);
          addedCount++;
