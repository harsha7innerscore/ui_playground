...
class ChakraTestIdAdder:
    ...
    def process_file(self, file_path):
        ...
        # Process the file line by line to maintain correct structure
        lines = content.split('\n')
        modified_lines = []

        # Track line numbers where we've added test IDs
        processed_lines = set()
        ...
        for line_num, line in enumerate(lines):
            ...
            matches = list(re.finditer(jsx_tag_pattern, line))
            if not matches:
                modified_lines.append(line)
                continue
            ...
            for match in reversed(matches):
                ...
                # Replace the matched part in the line
                start, end = match.span()
                line = line[:start] + new_tag + line[end:]
