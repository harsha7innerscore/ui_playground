  ...
  useEffect(() => {
    if (!user) {
      ...
      const loadLoginMicrofrontend = () => {
        // Create script element to load the login microfrontend from its own server
        // It's already running independently on port 5173
        const script = document.createElement('script');
        script.src = 'http://localhost:5173/login-microfrontend.umd.js';
        script.async = true;
