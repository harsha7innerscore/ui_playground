...
# ================================================================
# OAuth-Compatible Tool Functions
# ================================================================

def create_tools_with_client(github_client) -> list:
    """
    Create tools list for OAuth mode (no submit_review tool).
    ...
    """
    # Return all tools except submit_review for OAuth mode
    oauth_tools = []
    for tool in TOOLS:
        if tool["name"] != "submit_review":
            oauth_tools.append(tool)
    return oauth_tools


def execute_tool_with_client(tool_name: str, tool_input: dict, github_client) -> str:
    """
    Execute a tool using a specific GitHub client (OAuth mode).
    ...
    Returns:
        String result for Claude
    """
    import json

    try:
        if tool_name == "get_pr_details":
            result = github_client.get_pr_details(
                owner=tool_input["owner"],
                repo=tool_input["repo"],
                pr_number=tool_input["pr_number"],
            )
            ...
            result = github_client.get_pr_files(
                owner=tool_input["owner"],
                repo=tool_input["repo"],
                pr_number=tool_input["pr_number"],
            )
            ...
            result = github_client.get_pr_commits(
                owner=tool_input["owner"],
                repo=tool_input["repo"],
                pr_number=tool_input["pr_number"],
            )
            ...
            # This shouldn't be called in OAuth mode, but handle gracefully
            return "ERROR: submit_review is not available in OAuth mode. Review will be submitted through the UI."
