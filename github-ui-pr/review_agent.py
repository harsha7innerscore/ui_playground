...
def review_pr_with_token(owner: str, repo: str, pr_number: int, github_client) -> dict:
    """
    Review PR with user's GitHub client (OAuth-based).
    Returns structured review data for preview instead of auto-submitting.

    Args:
        owner: GitHub repository owner
        repo: Repository name
        pr_number: PR number
        github_client: GitHubClient instance with user's access token

    Returns:
        dict with review data for frontend preview
    """
    try:
        from agent_tools import create_tools_with_client

        print(f"\n🤖 Starting OAuth PR review for {owner}/{repo}#{pr_number}")

        # Create tools with user's GitHub client
        tools = create_tools_with_client(github_client)
        ...
        messages = [
            {
                "role": "user",
                "content": (
                    f"Please analyze this pull request:\n\n"
                    f"Repository: {owner}/{repo}\n"
                    f"PR Number: {pr_number}\n\n"
                    f"Get the PR details and changed files, then prepare a review "
                    f"with specific line comments. Do NOT submit the review - "
                    f"just return the analysis and comments."
                )
            }
        ]

        # Modified system prompt for OAuth flow (no auto-submit)
        oauth_system_prompt = SYSTEM_PROMPT + """

## IMPORTANT - OAuth Mode Instructions

You are running in OAuth mode. Do NOT call submit_review automatically.
Instead, after analyzing the PR:

1. Get PR details and files as usual
2. Analyze the code thoroughly
3. Prepare your review comments
4. Return a summary of your findings

The user will preview your review before deciding to submit it."""
        ...
        for iteration in range(max_iterations):
            print(f"\n⚡ OAuth agent iteration {iteration + 1}/{max_iterations}")
            ...
            if response.stop_reason == "end_turn":
                ...
                return {
                    ...
                    "summary": review_summary or final_text or "Review analysis completed",
                    "total_files": 1,  # TODO: Track actual file count
                    "total_tool_calls": total_tool_calls
                }
                ...
                for block in response.content:
                    if block.type != "tool_use":
                        if block.type == "text" and block.text.strip():
                            ...
                            # Extract review comments and summary from text
                            if "REVIEW SUMMARY:" in block.text:
                                review_summary = block.text.split("REVIEW SUMMARY:")[-1].strip()
                        continue
                    ...
                    # Execute tool with user's GitHub client
                    from agent_tools import execute_tool_with_client
                    result = execute_tool_with_client(tool_name, tool_input, github_client)
        ...
        return {
            "success": True,
            "comments": review_comments,
            "summary": review_summary or "Review completed",
            "total_files": 1,
            "total_tool_calls": total_tool_calls
        }

    except Exception as e:
        print(f"❌ OAuth review error: {e}")
        return {
            "success": False,
            "error": str(e),
            "total_tool_calls": 0
